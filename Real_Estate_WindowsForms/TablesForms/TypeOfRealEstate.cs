...
{
    public partial class TypeOfRealEstate : Form
    {
        public static string tableName = "TypeOfRealEstate";
        SQLiteHelper dbHelper = new SQLiteHelper(tableName);

        public TypeOfRealEstate(bool isAdmin)
        {
            InitializeComponent();
            LoadData();
            ...
            if (isAdmin)
            {
                Size = new System.Drawing.Size(new Point(342, 597));
                AdminPanel.Visible = true;
            }
            else
            {
                Size = new System.Drawing.Size(new Point(342, 334));
        ...
        {
            string query = "SELECT * FROM TypeOfRealEstate";
        ...
        private void SearchBTN_Click(object sender, EventArgs e)
        {
            ...
            {
                if (FieldValidator.ValidateIntFields(out errorMessage, IDtextBox2))
                {
        ...
        {
            if (dbHelper.RecordExistsByValueTableColumn(Code, "RealEstate", "TypeOfRealEstateCode"))
            {
                MessageBox.Show("Запись с этим типом есть в таблице RealEstate.\nУдаление невозможно!");
                return;
            }
